...
class Gold(Unit):
    ...
    def extract(self):
        ...
        values_gold = Connection.create(SERVER_LEVEL).query(sql_gold)
        self.glod_get(common, values_gold)
        self.glod_consume(common, values_gold)

    def glod_get(self, common, sql):
        for gold in sql:
            if gold["num"] >=0:
    ...
    def glod_consume(self, common, sql):
        for gold in sql:
            if gold["num"] < 0:
                common["openid"] = gold["uid"]
...
class Other(Unit):
    ...
    def other_badge(self, common):
        ...
        values_badge = Connection.create(SERVER_LEVEL).query(sql_badge)
...
class Props(Unit):
    ...
    def props_get(self, common):
        sqls = {
            "card": "select a.playerid, a.uid, a.subid, a.card_key as propsid, a.num, a.fromid as get_wayid, a.get_tm, b.lv as level, b.vip as vip_level from {db_log}.log_get_card as a inner join {db_game}.helix_player as b on a.playerid=b.playerid and get_tm>='{props_start}' and get_tm<'{start_dt}'".format(**common),
            "equip": "select a.playerid, a.uid, a.subid, a.equip_key as propsid, a.num, a.fromid as get_wayid, a.get_tm, b.lv as level, b.vip as vip_level from {db_log}.log_get_equip as a inner join {db_game}.helix_player as b on a.playerid=b.playerid and get_tm>='{props_start}' and get_tm<'{start_dt}'".format(**common),
            "medal": "select a.playerid, a.uid, a.subid, a.medal_key as propsid, a.fromid, a.from_mapid as get_wayid, a.from_stageid, a.num, a.new_num, a.log_tm as get_tm, b.lv as level, b.vip as vip_level from {db_log}.log_get_medal as a inner join {db_game}.helix_player as b on a.playerid=b.playerid and log_tm>='{props_start}' and log_tm<'{start_dt}'".format(**common),
        }
        for k, sql in sqls.iteritems():
            ...
            for props_get in props_gets:
                ...
                common["type"] = props_get.get("fromid", "")
                common["extend_1"] = ""
                common["extend_2"] = ""
    ...
    def props_consume(self, common):
        sqls = {
            "equip": "select a.playerid, a.uid, a.subid, a.equip_key as propsid, a.fromid as consume_wayid, a.from_cardid as consume_wayclassid, a.log_tm, b.lv as level, b.vip as vip_level from {db_log}.log_use_equip as a inner join {db_game}.helix_player as b on a.playerid=b.playerid and log_tm>='{props_start}' and log_tm<'{start_dt}'".format(**common),
        ...
        for k, sql in sqls.iteritems():
            ...
            for props_consume in props_consumes:
                ...
                common["consume_wayclassid"] = props_consume.get("consume_wayclassid", props_consume["consume_wayid"])
