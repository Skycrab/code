...
alter table littleboy_user_remains add if not exists partition(year='$V_PARYEAR',month='$V_PARMONTH',day='$V_PARDAY') location '$V_PARYEAR/$V_PARMONTH/$V_PARDAY';
insert overwrite table littleboy_user_remains partition(year='$V_PARYEAR',month='$V_PARMONTH',day='$V_PARDAY')
...
(
  select
      ...
      '${V_MONDAY_0}' relate_date, -- 对比计算周期的结束日期
      province_code,
      province_name
  from
  (
    select
      ...
      start_date,
      end_date,
      cast(floor(datediff('${V_MONDAY_0}',start_date)/7) as int) group_num,
    ...
    (
      select
        ...
        type,
        order_money,
        date_add('2016-01-04',cast(floor(datediff(to_date(create_time),'2016-01-04')/7) as int)*7+0) start_date,
        date_add('2016-01-04',cast(floor(datediff(to_date(create_time),'2016-01-04')/7) as int)*7+6) end_date
    ...
    (
      ...
      where concat(year,month,day) between '${V_MONDAY}' and '${V_YESTERDAY}'
      and status=1
      group by user_id
    ) t2
